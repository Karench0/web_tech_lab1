...
function renderCategory({ data, containerSelector, category }) {
  ...
  const cardsHtml = items.map(item => `
    <div class="dish-card" data-kind="${item.type}">
      <img src="${item.img}" alt="${item.alt ?? 'Блюдо'}" height="450">
      <p class="price">${item.price} ₽</p>
      <p class="name">${item.name}</p>
      <p class="weight">${item.weight} ${unit}</p>
      <button id="${item.id}">Добавить</button>
    </div>
  `).join('');
